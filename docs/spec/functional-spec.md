...
- 2026-05-30 — v0.11 — Multi-device bug fixes from a two-sim playthrough. §3.2 lobby: re-tapping CALL FOR BOARDING after editing rules now preserves the room code (was regenerating it and orphaning any joiner in the lobby); ScrollView resets to the top on phase change so STEP 1 OF 2 is always visible on re-entry; rule-summary chip wraps to two lines instead of truncating and now covers every non-default rule. §7 join flow: late joiners (who connect after the host has departed) auto-route to the spectator view instead of getting stuck on "Waiting for the conductor to depart…"; nearby-host list labels generic "Conductor" entries as "Room {code}" so multiple games at one table are distinguishable; joiner name persists to settings on BOARD so a cancel-and-rejoin pre-fills the identity. §3.4 scoring: tapping "Enter manually instead" on the camera-permission gate is now a session-wide preference — subsequent ADD SCORE taps skip the camera entirely and route straight to manual entry. §3.7 end-game: header subtitle drops the date when the game has no custom name (was reading "May 30, 2026 / FINAL · May 30, 2026"). House-rules picker toggles now flip from a tap anywhere on the row, not just the small switch.
- 2026-05-30 — v0.10 — Blank-tile house rules. §2 gains three more family-rule options: Double-blank penalty (None / +25 / +50), Doubles count double (toggle — a `N|N` left in hand counts 2× its standard pip value), Any-blank penalty (None / +5 per blank tile half). Defaults remain "no special rules". Because the photo pip-counter can't reliably distinguish a 0|0 from another blank, the **double-blank penalty is applied via a round-end prompt** that fires after the last hand of the stop is entered ("Who had the 0|0?" → conductor picks a player or "Nobody had it"); the chosen player's stop score is adjusted with an audit entry. This prompt chains before the existing blocked-round-cap prompt. Doubles-count-double and any-blank penalty are surfaced as one-tap chips in manual entry — the doubles chip is a Menu of `0|0 … 12|12` that adds `+2N` extra pips per pick. §3.7 past-game view now shows a **RULES IN PLAY** card so totals can be interpreted in context.
- 2026-05-30 — v0.9 — Rules-first new-game flow + house-rules expansion. §3.2 splits New Game into two explicit phases: (1) Set the rules — game length, starting engine, and house rules — nothing broadcasts yet; (2) Call for boarding — QR + room code go live, the table can join, conductor can still tap EDIT RULES to flip back without dropping the room code. Four house rules added (§2): Going-out bonus (None / −5 / −10) is applied at score-entry time when the conductor enters 0 pips; Doubles penalty (None / +5 / +10) surfaces as a one-tap "+N PENALTY" chip in manual entry; Draw count (Auto / 7 / 10 / 12 / 15) overrides the round-instructions card on the scoreboard; Blocked-round cap toggles a confirmation on ADVANCE when nobody went out — confirming rewrites the lowest hand to 0 with an audit entry. Rules can be edited mid-game from the scoreboard's "Edit rules" menu and re-broadcast to joiners on the spot. Snapshot wire format gains the four fields with safe defaults for back-compat.
- 2026-05-30 — v0.8 — Polish pass driven by a full 8-player playthrough. §3.3 scoreboard: rank-suffix column widened so ordinals never wrap into "2n d"/"6t h"; header engine glyph enlarged for at-a-glance recognition; the one-shot "broadcasting" cue now appears as a bottom toast and uses generic copy so it can never cover the leader row (the room code remains visible in the header). §3.4 manual entry: the readout shows a placeholder dash until digits are entered (was a misleading "0" that made the disabled ALL ABOARD button look tappable). §3.2 new-game lobby: scroll content reserves room under the sticky DEPART footer so the third engine option and the manual-add field can always be brought into view; conductor row drops the redundant pencil affordance; "ADD player" pill is recolored to the high-contrast CTA treatment. §3.7 end-game: standings rows tightened so a max-player roster fits a single screen. Scorecard cells lock their height to remove per-row jitter; the past-game scorecard now opts into a roomier density. Camera permission grant is latched for the session so subsequent captures skip the gate; the "Enter manually instead" link grows to a 44pt tap target.
