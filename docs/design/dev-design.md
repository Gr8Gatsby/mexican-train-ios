...
- 2026-05-22 — v0.4 — In-app QR scanner shipped: `QRScannerView` wraps `AVCaptureMetadataOutput` and presents inside `JoinSheet` via a "SCAN QR" button next to the room-code field. Decodes either the full `mextrain://join?code=NNNN` URL or a bare 4-digit code, auto-connects when the host is in the available-hosts list. The iOS Camera-app path still works via the custom URL scheme (system shows a banner). Universal Links / AASA stay deferred (open question §11.5 unchanged — needs a controlled hostname). Custom fonts shipped: `Rye-Regular.ttf` + `SpecialElite-Regular.ttf` bundled under `MexicanTrain/Resources/Fonts/`, registered at launch via `Fonts.registerBundledFonts()`. License/attribution recorded in `LICENSE.fonts.txt` alongside the fonts.
